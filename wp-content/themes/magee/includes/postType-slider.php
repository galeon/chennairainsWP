<?php
 ...
 function magee_slider_register() {
	...
	$labels = array(
		'name' => __('Magee Slider',"magee"),
		'singular_name' => 'Magee Slider',
		'add_new_item' => __('Add New Slider',"magee"),
		'edit_item' => __('Edit Slider', 'magee'),
		'new_item' => __('New Slider', 'magee'),
		'view_item' => '',
		'all_items' => 'All Sliders',
		'search_items' => __('Search Slider', 'magee'),
	...
	$args = array(
		...
		'public' => false,
		'show_ui' => true,
		'menu_icon' => get_template_directory_uri().'/images/menu-icon-slider.png',
		'can_export' => true,
 ...
 function magee_slider_slides(){
	...
	$slider = unserialize( base64_decode($custom["magee_custom_slider"][0]) );
	...
	foreach( $slider as $slide ):
		...
		<li id="listItem_<?php echo $i ?>"  class="ui-state-default" style="margin-bottom:10px;">
			<div class="widget-content option-item" style="margin:5px; padding:5px;">
				...
				<label for="magee_custom_slider[<?php echo $i ?>][title]"><span><?php _e("Slide Title","magee");?> :</span><input id="magee_custom_slider[<?php echo $i ?>][title]" name="magee_custom_slider[<?php echo $i ?>][title]" value="<?php  echo stripslashes( $slide['title'] )  ?>" type="text" /></label>
				
				<label for="magee_custom_slider[<?php echo $i ?>][caption]"><span style="float:left" ><?php _e("Slide Caption","magee");?> :</span><textarea name="magee_custom_slider[<?php echo $i ?>][caption]" id="magee_custom_slider[<?php echo $i ?>][caption]"><?php echo stripslashes($slide['caption']) ; ?></textarea></label>
				<input id="magee_custom_slider[<?php echo $i ?>][id]" name="magee_custom_slider[<?php echo $i ?>][id]" value="<?php  echo $slide['id']  ?>" type="hidden" />
                <a class="sort-item"><?php _e("Sort","magee");?></a>
				<a class="del-item"><?php _e("Delete","magee");?></a>
				<div class="clear"></div>
			</div>
		</li>
	<?php endforeach; 
	}else{
		echo ' <p> '.__("Use the button above to add Slides !","magee").'</p>';
	}?>
	</ul>
	<div class="clear"></div>
  ...
  <?php
	...
	function magee_save_slide(){
	  ...
	  if(isset($post->ID) && isset($_POST['magee-custom-slider'])){
		if( isset($_POST['magee_custom_slider']) && $_POST['magee_custom_slider'] != "" ){
			$magee_custom_slider = base64_encode(serialize($_POST['magee_custom_slider']));
	...
	function magee_slider_edit_columns($columns){
	  $columns = array(
		...
		"slides" => __("Number of Slides","magee"),
		"date" => __("Date","magee"),
	  );
