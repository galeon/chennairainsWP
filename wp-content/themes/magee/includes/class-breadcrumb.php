...
if(!class_exists('magee_breadcrumb'))
{

	class magee_breadcrumb
	{
		var $options;
	...
	function magee_breadcrumb($options = ""){
		...
		if(is_404()){ return _e("404 - Page not Found",'magee_framework'); }
		
		if(is_search()){ return _e("Search",'magee_framework');}	
...
/*-----------------------------------------------------------------------------------*/
/* magee_breadcrumbs() - Custom breadcrumb generator function  */
/*
/* Params:
/*
/* Arguments Array:
...
function magee_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&raquo;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'magee') . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'magee'),
		'echo' => false, 
	...
	$args = apply_filters( 'magee_breadcrumbs_args', $args );
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, magee_breadcrumbs_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( !empty( $path ) )
				$trail = array_merge( $trail, magee_breadcrumbs_get_parents( '', $path ) );
		...
		*/
		...
		if ( empty( $path ) && 0 !== $parent || 'attachment' == $post_type )
			$trail = array_merge( $trail, magee_breadcrumbs_get_parents( $parent, '' ) );
		...
		if ( 'post' == $post_type && $show_posts_page == true && 'page' == get_option( 'show_on_front' ) ) {
			...
			if ( $posts_page != '' && is_numeric( $posts_page ) ) {
				$trail = array_merge( $trail, magee_breadcrumbs_get_parents( $posts_page, '' ) );
			}
		}
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, magee_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'magee') );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'magee'), get_the_time( __( 'i', 'magee') ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'magee') );
		}
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, magee_breadcrumbs_get_parents( '', $wp_rewrite->front ) );

			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'magee') ) . '">' . get_the_time( __( 'Y', 'magee') ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'magee') ) . '">' . get_the_time( __( 'F', 'magee') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'magee') );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'magee') ) . '">' . get_the_time( __( 'Y', 'magee') ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'magee'), get_the_time( esc_attr__( 'W', 'magee') ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'magee') ) . '">' . get_the_time( __( 'Y', 'magee') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'magee') );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'magee') );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'magee'), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'magee');

	/* Allow child themes/plugins to filter the trail array. */
	$trail = apply_filters( 'magee_breadcrumbs_trail', $trail, $args );
	...
	$breadcrumb = apply_filters( 'magee_breadcrumbs', $breadcrumb );
...
} // End magee_breadcrumbs()


function magee_breadcrumbs_get_parents( $post_id = '', $path = '' ) {
...
} // End magee_breadcrumbs_get_parents()


function magee_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '' ) {
...
} // End magee_breadcrumbs_get_term_parents()